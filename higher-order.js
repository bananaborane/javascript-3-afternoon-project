...
let evenArrayMaker = function(element){return element%2===0};
let evenNumbers = mixedNumbers.filter(evenArrayMaker)
...
let taxedAdderToPrice = (element)=>{return element*=1.07}
let postTaxPrices = prices.map(taxedAdderToPrice);
...
let sumThemUp = (runningTotal, currentElement, currentIndex, wholeArray)=>{return runningTotal + currentElement}
let totalPopulation = populations.reduce(sumThemUp)
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(value, i, arr){
  return value.CP > 200;
})
...
let orderTotals = orders.map((element, i  , arr)=>{return element.price*(1+element.tax)})
...
// let bobsArray = purchases.filter((element)=>{
//   return element.owner === 'Bob'}).reduce((total, element)=>{
//     return total + element.price;  
// // let bobsTotal = bobsArray.reduce((total, element)=>{
// //   return total + element.price;  
// })
